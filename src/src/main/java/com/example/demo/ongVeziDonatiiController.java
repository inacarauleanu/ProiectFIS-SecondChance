...
public class ongVeziDonatiiController implements Initializable{
    @FXML
    ...
    ObservableList<DonatieSH> DonatiiONGList = FXCollections.observableArrayList();
    ...
    private void refreshTable() {
        try {
            DonatiiONGList.clear();
            //CLIENT client = new();
            query = "select * from donatii_ong WHERE usernameONG = '" + username  + "'";
            preparedStatement = connection.prepareStatement(query);
            resultSet = preparedStatement.executeQuery();

            while(resultSet.next())
            {
                DonatiiONGList.add(new DonatieSH(resultSet.getInt("id_donatie"),
                        resultSet.getString("productName"),
                        resultSet.getString("productSize"),
                        resultSet.getString("productColor"),
                        resultSet.getString("date")));
                donatiiONGTable.setItems(DonatiiONGList);
